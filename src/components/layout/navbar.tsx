	...
	return (
		<div className="relative">
			<header className="sticky top-0 z-50 bg-primary-white w-full transition-all duration-300 shadow-sm">
				<div className={`mx-auto max-w-[1440px] px-6 md:px-20 py-3 flex bg-primary-white items-center justify-between`}>
					<div className="flex items-center">
						<Link href="/" className="flex items-center">
							...
							/>
							<span className="ml-3 text-lg lg:text-2xl font-bold text-primary-maroon">NK Engineering</span>
						</Link>
					</div>
					...
					>
						...
						<span className={`block h-0.5 w-6 bg-black transition-opacity duration-300 ${isMenuOpen ? 'opacity-0' : 'opacity-100'}`}></span>
						<span className={`block h-0.5 w-6 bg-black transition-transform duration-300 ${isMenuOpen ? '-rotate-45 -translate-y-2' : ''}`}></span>
					</button>
				</div>
				
				{/* Mobile Navigation */}
				<div className={`fixed inset-0 bg-white z-40 md:hidden flex flex-col justify-center items-center space-y-8 transform transition-transform duration-300 ${isMenuOpen ? 'translate-y-0' : '-translate-y-full'}`}>
					{[
						{ href: "/", label: "Home" },
						{ href: "/about", label: "About Us" },
						{ href: "/products", label: "Products" },
